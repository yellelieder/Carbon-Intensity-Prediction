...
def evaluate_model(training_data_file_path, intervalls, model):
    ...
    model = sm.load(f"{config.model_folder}ModelsAutoRegression{col}{config.slash}{model}")
...
if __name__ == "__main__":
    evaluate_model("Ressources{config.slash}TrainingData{config.slash}Production.csv", intervalls=7*96, model="3.pickles")
    evaluate_model("Ressources{config.slash}TrainingData{config.slash}Consumption.csv", intervalls=7*96, model="4.pickles")
