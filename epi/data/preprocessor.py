    ...
    if type == config.p_id:
        ...
        for column in columns:
            df[column] = (
                (
                    df[column].apply(
                        lambda outer_value: str(outer_value).replace(".", "")
                    )
                )
                .apply(lambda inner_value: str(inner_value).replace(",", "."))
                .apply(lambda y: int(float(str(y))))
            )
            df[column == 0] = np.nan
        ...
        # removing thousand seperators and replacing decimal colons with points
        df[mwh] = (
            (df[mwh].apply(lambda outer_value: str(outer_value).replace(".", "")))
            .apply(lambda inner_value: str(inner_value).replace(",", "."))
            .apply(lambda y: int(float(str(y))))
        )
        df[mwh == 0] = np.nan
