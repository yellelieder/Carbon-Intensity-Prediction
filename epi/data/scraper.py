...
def _scrape(type:str):
    ...
    download_path=config.download_production_folder if type==config.p_id else config.download_consumption_folder
    preferences={"download.default_directory":config.local_path+f"EPI{config.slash}"+download_path}
