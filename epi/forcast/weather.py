...
def _get_forcast(lat, lng) -> json:
    ...
    try:
        ...
        print(
            "openweathermap.org weather forcast was not succefull, first check api keys"
        )
...
def get_best_start(lat, lon, start: str, end: str, dur: int):
    ...
    sug = common.str_to_datetime(
        datetime.utcfromtimestamp(pred[start_hour]["dt"]).strftime("%d/%m/%Y")
        + " "
        + surise
        + ":00"
    )
